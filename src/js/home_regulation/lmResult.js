...
export default {
  ...
  methods: {
    ...
    //isShow
    switchdown(index,item){
       item.isShow = !item.isShow;
        if(item.isShow == true){
          this.$refs.iconBox[index].style.transform = 'rotate(-90deg)';
        }else{
          this.$refs.iconBox[index].style.transform = 'rotate(-180deg)';
        }
    },
    //选择时间&球类赛果时把icon退回
    iconDown(){
  ...
  mounted(){
    this.$nextTick(function () {
      //发送请求
      base.methods.getResult({date:this.newTime == '' ? tools.formatDate(new Date().getTime()- 12 * 60 * 60 * 1000,'YY-MM-DD') : this.newTime,currPage:'1',lottery:'31'});
      if(!this.scroll) {
